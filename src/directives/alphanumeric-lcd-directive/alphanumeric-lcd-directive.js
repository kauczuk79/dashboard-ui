    ...
    function AlphanumericLcdDirective() {
        function link(scope, element, attrs) {
            ...
            for (lineIterator = 0; lineIterator < scope.parameters.rows; lineIterator += 1) {
                ...
                lcdGroup.append('text').classed(FOREGROUND_CLASS, true).attr('y', yPosition).prependScale(scope.parameters.scale);
                if (scope.parameters.showBackground) {
                    lcdGroup.append('text').classed(BACKGROUND_CLASS, true).attr('y', yPosition).prependScale(scope.parameters.scale).data(RECTANGLE_CHAR).text(function (data) {
                        var arr = [];
